...
description: "Modify fields or replace files on an existing product by ID. Requires admin level 2 authentication and accepts multipart/form-data."
---
...
<Note>
  The server calculates and stores `discountPrice` only when **all three**
  conditions in the controller are met:

  ```js
  typeof discount === 'string' && !isNaN(parseFloat(discount)) && isFinite(price)
  ```

  In practice this means: `discount` must be sent as a numeric string (e.g.
  `"10"`, `"25.5"`), **and** a finite numeric `price` must also be included
  in the same request body. When both are present, the server sets:
